...
import { selectProject, deleteProject } from "@/store/lavaSlice";
  ...
  if (!project) {
    return (
      <div className="flex flex-col items-center justify-center h-64 space-y-4">
        ...
        <p className="text-slate-500 pt-10 font-medium">
          No active project found. Please select or create one.
        </p>
          ...
          {showProjectPicker && (
            <>
              ...
              <div className="absolute top-12 left-0 w-64 bg-slate-900 border rounded-2xl shadow-xl z-50 p-2 space-y-1">
                <p className="text-[10px] font-bold text-slate-400 p-2 uppercase">
                  Your Projects
                </p>
        ...
        <>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 mt-1">
                Aggregated data from your current project.
              </p>
            </div>
            ...
            <button
              onClick={() => dispatch(deleteProject())}
              className="flex items-center gap-2 mx-4 px-4 py-2 bg-slate-800 rounded-xl font-bold hover:bg-slate-700 transition-colors"
            >
              delete project
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {stats.map((stat, i) => (
              ...
              >
                <div
                  className={`w-12 h-12 rounded-2xl ${stat.color} flex items-center justify-center mb-4 transition-transform group-hover:scale-110`}
                >
                  {stat.icon}
                </div>
